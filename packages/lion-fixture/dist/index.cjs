...
var path__namespace = /*#__PURE__*/_interopNamespace(path$3);
...
function lionFixture(options) {
    let fixturesDir;
    let tempDir;
    if (typeof options === 'string') {
        let fileUrl = options;
        if (fileUrl.startsWith('file://')) {
            fileUrl = url.fileURLToPath(fileUrl);
        }
        fixturesDir = path__namespace.join(path__namespace.dirname(fileUrl), '../fixtures');
        tempDir = path__namespace.join(path__namespace.dirname(fileUrl), '../temp');
    }
    else {
    ...
    async function fixture(fixtureName, tempFixtureNameOrOptions, maybeFixtureOptions) {
        ...
        if (fixturesDir.startsWith('file://')) {
            fixturesDir = url.fileURLToPath(fixturesDir);
        }
        await fs__namespace.promises.mkdir(tempDir, { recursive: true });
        const originalFixtureDir = path__namespace.join(fixturesDir, fixtureName);
        const tempFixtureDir = path__namespace.join(tempDir, tempFixtureName);
        // Remove the temporary fixture directory if it already exists
        if (fs__namespace.existsSync(tempFixtureDir)) {
            await fs__namespace.promises.rm(tempFixtureDir, { recursive: true, force: true });
        }
        await fs__namespace.promises.cp(originalFixtureDir, tempFixtureDir, {
            recursive: true,
        });
        if (!fixtureOptions.runInstall &&
            fs__namespace.existsSync(path__namespace.join(tempFixtureDir, 'package.json'))) {
    ...
    function fixtureSync(fixtureName, tempFixtureName) {
        ...
        const originalFixtureDir = path__namespace.join(fixturesDir, fixtureName);
        const tempFixtureDir = path__namespace.join(tempDir, tempFixtureName ?? fixtureName);
        fs__namespace.cpSync(originalFixtureDir, tempFixtureDir, { recursive: true });
